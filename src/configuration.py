...
from src.constant import CONFIG_FILE, DATETIME_FORMAT
...
class Configuration:
    ...
    def __init__(self, logger=None):
        ...
        if self.configurations["start_time"] >= self.configurations["end_time"]:
            print_and_log(self.logger, "error", f"Error validating the start_time. The start_time: {self.configurations['start_time']} cannot be greater than or equal to the end_time: {self.configurations['end_time']}")
            exit(0)
        # Converting datetime object to string
        for date_config in ["start_time", "end_time"]:
            self.configurations[date_config] = self.configurations[date_config].strftime(DATETIME_FORMAT)
    ...
    def validate(self):
        ...
        if validator.errors:
            print_and_log(
                self.logger,
                "error",
                "Error while validating the config. Errors: %s" % (validator.errors)
            )
            exit(0)
