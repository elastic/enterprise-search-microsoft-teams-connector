...
PIP = pip3
VENV_DIRECTORY = venv
PROJECT_DIRECTORY = ees_microsoft_teams
...
help:
	...
	@echo "make test_connectivity - test connectivity to Microsoft Teams and Enterprise Search"
...
ifneq ($(OS),Windows_NT)
	VENV_FULL_PATH = ${PWD}/${VENV_DIRECTORY} | sed 's/ /\\ /g'
endif

.installed: .venv_init
	${VENV_FULL_PATH}/${EXEC_DIR}/${PIP} install -U pip
	${VENV_FULL_PATH}/${EXEC_DIR}/${PIP} install -r requirements.txt
...
install_locally: .installed .venv_init
	${VENV_FULL_PATH}/${EXEC_DIR}/${PIP} install .

test: .installed .venv_init
	${VENV_FULL_PATH}/${EXEC_DIR}/${PYTHON_EXE} -m pytest ${TEST_DIRECTORY}/ --suppress-no-test-exit-code
...
lint: .installed .venv_init
	${VENV_FULL_PATH}/${EXEC_DIR}/flake8 ${PROJECT_DIRECTORY}

test_connectivity: .installed .venv_init
	${VENV_FULL_PATH}/${EXEC_DIR}/pytest ${PROJECT_DIRECTORY}/test_connectivity.py
