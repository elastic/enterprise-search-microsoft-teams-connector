...
class IncorrectFormatError(Exception):
    ...
    def __init__(self, obj_type, checkpoint, inner_exception):
        super().__init__(
            f"Start time: {checkpoint} for {obj_type} in the checkpoint file {CHECKPOINT_PATH} is not in the correct "
            f"format. Expected format: {constant.DATETIME_FORMAT}. Remove the checkpoint entry for the {obj_type} "
...
class Checkpoint:
    ...
    def get_checkpoint(self, current_time, obj_type):
        """ This method fetches the checkpoint from the checkpoint file in
            ...
            :param obj_type: Microsoft Teams for which checkpoint is fetched
        """
        self.logger.info(
            f"Fetching the checkpoint details for {obj_type} from the checkpoint file: {CHECKPOINT_PATH}"
        )
        ...
        if (os.path.exists(CHECKPOINT_PATH) and os.path.getsize(CHECKPOINT_PATH) > 0):
                    ...
                    if not checkpoint_list.get(obj_type):
                        ...
                        try:
                            start_time = coerce_rfc_3339_date(
                                checkpoint_list.get(obj_type)).strftime(
            ...
            self.logger.debug(
                f"Checkpoint file does not exist at {CHECKPOINT_PATH}, considering the start_time and end_time from "
    ...
    def set_checkpoint(self, current_time, index_type, obj_type):
        ...
        try:
            with open(CHECKPOINT_PATH, encoding="UTF-8") as checkpoint_store:
                ...
                if checkpoint_list.get(obj_type):
                    self.logger.debug(
                        f"Setting the checkpoint contents: {current_time} for the {obj_type} to the checkpoint path: "
                        f"{CHECKPOINT_PATH}"
                    )
                    checkpoint_list[obj_type] = current_time
                else:
                    self.logger.debug(
                        f"Setting the checkpoint contents: {self.config.get_value('end_time')} for the {obj_type} to "
                        f"the checkpoint path: {CHECKPOINT_PATH}"
                    )
            ...
            if isinstance(exception, FileNotFoundError):
                ...
                self.logger.exception(
                    f"Error while fetching the json file of the checkpoint store from path: {CHECKPOINT_PATH}. Error: "
                    f"{exception}"
                )
            ...
            self.logger.debug(
                f"Setting the checkpoint contents: {checkpoint_time} for the {obj_type} to the checkpoint path: "
                f"{CHECKPOINT_PATH}"
            )
        ...
        with open(CHECKPOINT_PATH, "w", encoding="UTF-8") as checkpoint_store:
            try:
                ...
                self.logger.exception(
                    "Error while updating the existing checkpoint json file. Adding the new content directly instead "
                    f"of updating. Error: {exception}"
                )
