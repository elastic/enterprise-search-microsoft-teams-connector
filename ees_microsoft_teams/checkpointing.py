...
class IncorrectFormatError(Exception):
    ...
    def __init__(self, obj_type, checkpoint, inner_exception):
        ...
        Expected format: {constant.DATETIME_FORMAT}. Remove the checkpoint entry for the {obj_type} or fix the format to continue indexing")
            ...
            with open(CHECKPOINT_PATH, encoding="UTF-8") as checkpoint_store:
                try:
                    ...
                    if not checkpoint_list.get(obj_type):
                        ...
                        try:
                            start_time = coerce_rfc_3339_date(checkpoint_list.get(obj_type)).strftime(constant.DATETIME_FORMAT)
