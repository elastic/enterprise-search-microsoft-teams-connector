...
class Indexer(BaseClass):
    ...
    def index_teams_and_children(self, teams_channels_obj, is_error_shared):
        """ This method is used to index teams, channels and it's objects data
            :param teams_channels_obj: MS Teams channels objects
            :param is_error_shared: :List of all the is_error values
        """
        logger.info("Start fetching and indexing the teams and it's objects data...")
        ...
        try:
            ...
            teams, self.is_error = teams_channels_obj.get_all_teams(doc_ids_storage)
            if "teams" in self.objects:
                self.bulk_index_document(teams, constant.TEAMS)
            if not self.is_error:
                channels, documents, self.is_error = teams_channels_obj.get_team_channels(teams, doc_ids_storage)
                if "channels" in self.objects:
                    self.bulk_index_document(documents, constant.CHANNELS)
            if "channel_messages" in self.objects and not self.is_error:
                channel_messages, _ = teams_channels_obj.get_channel_messages(channels, doc_ids_storage)
                self.bulk_index_document(channel_messages, constant.CHANNEL_MESSAGES)
            if "channel_tabs" in self.objects and not self.is_error:
                channel_tabs, _ = teams_channels_obj.get_channel_tabs(channels, doc_ids_storage)
                self.bulk_index_document(channel_tabs, constant.CHANNEL_TABS)
            if "channel_documents" in self.objects and not self.is_error:
                channel_documents, _ = teams_channels_obj.get_channel_documents(teams, doc_ids_storage)
                self.bulk_index_document(channel_documents, constant.CHANNEL_DOCUMENTS)
            ...
            with open(constant.CHANNEL_CHAT_DEINDEXING_PATH, "w", encoding="UTF-8") as f:
                try:
                    ...
                    logger.warn('Error while adding document ids to json file. Error: %s' % (exception))
        except Exception as exception:
            print_and_log(
                self.logger,
                "exception",
                "[Fail] Error while indexing the teams or it's objects data. Error: %s" % (exception),
            )
        logger.info("Completed indexing teams and it's objects data to the Workplace Search")
        is_error_shared.append(self.is_error)

    def index_teams_channels(self, is_error_shared):
        ...
        teams_channels_obj = MSTeamsChannels(self.access_token, self.start_time, self.end_time, self.get_schema_fields, logger)
        if any(obj in self.objects for obj in obj_permissions_list):
            if self.permission:
            ...
            self.index_teams_and_children(teams_channels_obj, is_error_shared)
...
def init_multiprocessing(access_token, start_time, end_time, type_, is_error_shared, user_drive):
    ...
    if type_ == "teams":
        indexer.index_teams_channels(is_error_shared)
