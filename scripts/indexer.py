...
class Indexer(BaseClass):
    ...
    def index_user_chats(self, is_error_shared, user_drive):
        ...
        """
        logger.info("Started indexing user chat, meeting chat, attachments, tabs and meeting recoding")
        ...
        try:
            ...
            user_msg = ms_msg.MSTeamsUserMessage(self.access_token, self.start_time, self.end_time, self.get_schema_fields, logger)
            user_permisssions, documents, self.is_error = user_msg.get_chats(doc_ids_storage, user_drive)
            ...
            self.bulk_index_document(documents, constant.USER_CHATS_MESSAGE)
        ...
        logger.info("Completed indexing user chat, meeting chat, attachments, tabs and meeting recoding")
...
def init_multiprocessing(access_token, start_time, end_time, type_, is_error_shared, user_drive):
    ...
    if type_ == "user_chats":
        indexer.index_user_chats(is_error_shared, user_drive)
